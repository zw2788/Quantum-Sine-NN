    ...
    def __init__(self, seed=None, num_neuron=None, num_hidden_layers=1, mode='constant', w_0 = 10):
        ...
        for i in range(max(num_hidden_layers - 1, 0)):
            if mode == 'constant':
                ...
                if current_neurons > 2:
                    next_neurons = current_neurons - 1
                else:
                    next_neurons = 2  # stay at 2 once reached
            else:
    ...
    def __init__(self, seed=None, num_neuron=4, num_hidden_layers=1, mode='constant', w_0=10):
        ...
        for i in range(max(num_hidden_layers - 1, 0)):
            if mode == 'constant':
                ...
                next_neurons = max(5, current_neurons - 1)
            else:
                raise ValueError("Mode must be 'constant' or 'decreasing'")
...
def train_model(
    ...
    model = BinarizedSinModel(seed=seed, num_neuron=num_neuron, num_hidden_layers=num_hidden_layers, mode= mode, w_0= w_0)
...
def generate_random_split(seed=42, start = -7 * PI/2, end= 7 * PI/2, train_ratio=0.8, total_points=100):
    ...
    all_targets = torch.sin(all_inputs) + torch.sin(2*all_inputs) + torch.sin(3*all_inputs)+ torch.sin(4*all_inputs)+ torch.sin(5*all_inputs)
...
for num_neuron in range(12, 18):  # 2, 3, ..., 7, for paper range(2,12)
    for num_hidden_layers in range(1, 11): # for paper range(1,6)
        ...
        for seed in range(0, 200):  # Adjust the range for more seeds
            ...
            targets=targets, num_epochs=300, lr=0.002, print_every=100, w_0 = 1)
...
with open('proportions_layerN14L10.txt', 'w') as f:
...
with open('all_train_test_lossN14L10.csv', 'w', newline='') as csvfile:
